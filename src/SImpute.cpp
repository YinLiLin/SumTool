...
SEXP SImputeZ_bin_c(XPtr<BigMatrix> pMat, const IntegerVector typed_index, const arma::mat typed_value, const double lambda = 0.0, const double maf = 0.01, const bool haps = false, const bool verbose = true, const int threads = 0){
	...
	if(haps){
		...
		#pragma omp parallel for schedule(dynamic) private(j, i, x11, x12, x21, x22, k, gi, gj, p1, p2, q1, q2)
		for (j = 0; j < m; j++){
			for(i = j + 1; i < m; i++){
				...
				q1 = x11 + x21;
				q2 = x12 + x22;
				Rtt(i, j) = (x11 * x22 - x12 * x21) / sqrt(p1 * p2 * q1 * q2);
				Rtt(j, i) = Rtt(i, j);
			}
			Rtt(j, j) = 1 + lambda;
		}
	...
	if(haps){
			...
			if ( ! Progress::check_abort() ) {
				...
				if(p1 < maf || p1 > (1 - maf)){
					...
					for(i = 0; i < m; i++){
						...
						q1 = x11 + x21;
						q2 = x12 + x22;
						Rit[i] = (x11 * x22 - x12 * x21) / (sqrt(p1 * p2 * q1 * q2));
					}
					beta = Rit.t() * iRtt;
		...
		#pragma omp parallel for schedule(dynamic) private(j, s1, s2, p1, m1, i, k, p12, p2, m2, r, beta)
		for(j = 0; j < nontyped_index_.size(); j++){
...
SEXP SImpute_joint_c(XPtr<BigMatrix> pMat, const IntegerVector typed_index, const arma::mat typed_value, const double maf = 0.01, const bool verbose = true, int threads = 0){
	...
	#pragma omp parallel for schedule(dynamic) private(j, p1, m1, s1, i, k, p12, p2, m2, s2, r)
	for(j = 0; j < n; j++){
		if(freq_all[j] < maf || freq_all[j] > (1 - maf))	continue;
		arma::vec Rit(m);
		p1 = sd_all[j];
		m1 = mean_all[j];
		s1 = sum_all[j];
