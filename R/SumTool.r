...
function(
	...
	map <- read.table(paste(bfile, ".bim", sep=""), header=FALSE)[,c(2, 1, 4, 5, 6)]
	pheno <- read.table(paste(bfile, ".fam", sep=""), header=FALSE)[,-c(1:5), drop=FALSE]
	m <- nrow(map)
	n <- nrow(pheno)
	...
	Data_c(bfile = bfile, pBigMat = geno@address, maxLine = maxLine, verbose = verbose, threads = threads)
	colnames(map) <- c("SNP", "Chr", "Pos", "A1", "A2")
...
SImputeZ <- function(ref.geno = NULL, ref.map = NULL, typed.geno = NULL, typed = NULL, w = 1000000, b = 125000, lambda = NULL, maf = 0.000001, correlation = TRUE, verbose = TRUE, threads = 1)
{
	...
	ref.map[, 3] <- as.numeric(as.character(ref.map[, 3]))
	typed[, 3] <- as.numeric(as.character(typed[, 3]))
	...
	ref.map[,1] <- as.character(ref.map[,1])
	ref.map[,4] <- as.character(ref.map[,4])
	ref.map[,5] <- as.character(ref.map[,5])
	...
	if(!all(typed_index)){
		...
		if(verbose)	cat("(Warning: Some SNPs sharing same names between reference and typed don't have equal map information!)\n")
	}
...
SBLUP <- function(sumstat = NULL, geno = NULL, map = NULL, lambda = NULL, w = 1e6, threads = 1, verbose = TRUE){
	...
	map[, 3] <- as.numeric(as.character(map[, 3]))
	sumstat[, 3] <- as.numeric(as.character(sumstat[, 3]))
	...
	if(!all(indx)){
		...
		if(verbose)	cat("(Warning: Some SNPs sharing same names between reference and sumstat don't have equal map information!)\n")
	}
...
LDreg <- function(sumstat = NULL, ldscore = NULL, wld = NULL, maxz2 = 80, maf = 0.05, nblock = 200, verbose = TRUE){
	...
	h2_est <- function(sumstat, ldscore, wld = NULL, maxz2, maf, nblock, verbose){
		...
		indx <- apply(row_logi, 1, all); rm(row_logi); gc()
		if(!all(indx)){
			if(verbose)	cat("(Warning: Some SNPs sharing same names between sumstat and ldscore don't have equal map information!)\n")
			ldscore <- ldscore[indx, ]
			wld <- wld[indx, ]
			sumstat <- sumstat[indx, ]
		}
